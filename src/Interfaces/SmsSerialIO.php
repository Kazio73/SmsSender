...
// 3) nic nie pasuje
    ...
    /*   dio_fcntl($this->serial, F_SETFL, O_NONBLOCK);

        $w = dio_write($this->serial, "AT\r");
        error_log("dio_write bytes=" . var_export($w, true));

        $buf = '';
        $deadline = microtime(true) + 20.0; // 1s

        while (microtime(true) < $deadline) {
            $chunk = dio_read($this->serial, 1024);

            if ($chunk === false) {
                throw new RuntimeException("dio_read() returned false");
            }

            if ($chunk !== '') {
                $buf .= $chunk;
                if (str_contains($buf, "OK") || str_contains($buf, "ERROR")) {
                    break;
                }
            } else {
                usleep(20_000);
        ...
        error_log("RX HEX=" . bin2hex($buf) . " RX=" . str_replace(["\r","\n"], ['<CR>','<LF>'], $buf));*/
        ...
        while (time() < $endTime) {
            ...
            $bytes = array_map(fn($s) => trim($s), $bytes);

            // 1) ERROR ma priorytet globalnie
            foreach ($bytes as $byte) {
                if (stripos($byte, 'ERROR') !== false) {
                    return 'ERROR';
                }
            }
            foreach (array_reverse($bytes) as $byte) {
                ...
                foreach ($this->valid as $valid) {
                    if (stripos($byte, $valid) !== false) {
                        return $byte; // <- return only last line
                    }
                }
            }
            usleep(1000); // Sleep for 100ms to prevent busy waiting
            }
            return 'TIMEOUT';
        }
